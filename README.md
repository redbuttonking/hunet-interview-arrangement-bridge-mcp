...
- `READY_TO_SCHEDULE` 상태에서 사용자가 나인하이어로 직접 확정한 일정도 Slack 알림으로 기록한다. 확정 알림에서는 새 다우오피스 조회를 하지 않고 마지막 동기화 데이터를 사용한다. 가능한 인터뷰 회의실이 하나면 로컬 배정으로 자동 기록하고, 여러 개면 회의실 선택 대기 항목을 만든다. 동기화 이력이 없거나 가능한 회의실이 없으면 추측하지 않고 검토 대상으로 남긴다.
