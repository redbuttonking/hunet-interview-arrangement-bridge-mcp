...
나인하이어 Slack 알림으로 들어온 후보자 메시지는 현재 일정이 있는 후보자에 한해 자동 감지합니다. 문구만으로는 인터뷰 포기와 재조율 요청을 구분할 수 없으므로 일정·회의실·Slack 메시지를 자동 변경하지 않고 `CANDIDATE_MESSAGE_REVIEW_REQUIRED` 검토 건만 만듭니다.

- 대시보드와 `resolve_candidate_interview_absence_review`에서 후보자 원문, 기존 제안 일정, 추천 조치를 보여 준 뒤 기존 가능 시간으로 재조율, 면접관 일정 재수집 후 재조율, 취소, 보류를 명시적으로 선택합니다.
...
캘린더 제목의 채용명은 나인하이어 원본 채용명과 달라도 된다. 후보자명·날짜·시각이 현재 조율 건 중 정확히 하나에 대응하거나, 같은 이름의 조율 시작 검토 건이 하나뿐이고 장소가 확인되면 다우오피스 캘린더를 최종 확정 근거로 기록한다. 같은 이름 후보자가 여러 건이면 자동 확정하지 않는다. 장소는 열정룸·행복룸·게임체인저·의문당·혁신룸·상상룸을 인식한다.
...
> 현재 단계는 **MCP 서버 + Slack 테스트 앱 + 로컬 워커 + 다우오피스 회의실 조회 + 로컬 운영 대시보드**입니다. 후보자에게 나인하이어 일정 제안을 발송하는 실제 화면 자동화는 전용 Chrome 로그인과 화면 구조 확인 뒤에만 활성화합니다. 비밀번호는 저장하지 않습니다.
...
- 시간·회의실 추천을 확정할 때 후보자에게 제안할 일정을 최소 1개 이상 선택할 수 있습니다. 기본 선택은 서로 다른 날짜의 2개 일정이며, 1개 또는 3개 이상도 가능합니다. 연속 인터뷰는 날짜별 여러 회의실 배정을 하나의 후보자 제안으로 묶어 관리합니다.
- 나인하이어 자동화 로그인 버튼과 `open_ninehire_login`은 개인 브라우저와 분리된 Chrome 프로필을 엽니다. 최초 로그인은 사용자가 직접 수행하며, 로그인 정보는 코드·환경 변수·DB에 저장하지 않습니다.
