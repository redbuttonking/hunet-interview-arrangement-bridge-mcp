...
> 나인하이어·Slack·다우오피스를 연결해 채용 인터뷰 조율의 반복 업무를 줄이는 단일 PC용 운영 도구입니다.

## 현재 제공 범위

- **MCP 서버**. Codex가 후보자, 평가, 면접관 일정, 회의실, 확정 인터뷰 상태를 조회하고 승인된 업무를 처리합니다.
- **워커**. Slack 알림, 나인하이어 평가·확정 일정, 다우오피스 회의실·캘린더를 주기적으로 동기화합니다.
- **대시보드**. 지금 처리할 후보자, 면접관 일정 제출 현황, 회의실 후보, 확정 인터뷰, 예외 상황을 한 화면에서 확인하고 필요한 승인만 처리합니다.
- **Windows 설치형 실행**. 한 명의 채용 담당자가 자신의 PC에서 로컬 DB와 전용 Chrome 프로필을 사용하도록 구성할 수 있습니다.
...
외부 메시지 발송, 후보자 일정 제안, 인터뷰 취소·변경은 항상 초안 또는 대시보드 승인 단계를 거칩니다. 후보자 메시지의 의미가 모호할 때도 자동 취소하지 않고 재조율·인터뷰 종료·보류 중 운영자가 선택하도록 합니다.

## 설치형 사용 방법

`release/HunetInterviewOps-Setup.exe`를 실행하면 현재 사용자의 로컬 앱 폴더에 다음 구성요소가 설치됩니다.

- 대시보드 서버. `127.0.0.1:3100`에서만 실행됩니다.
- Windows 로그인 시 자동 시작되는 동기화 워커.
- 다우오피스·나인하이어 자동화를 위한 분리 Chrome 프로필.
- 최초 설치 시에만 가져오는 `.env`와 로컬 SQLite DB.

기존 설치본 위에 다시 설치하면 기존 `.env`와 `data/bridge.db`는 유지하고 프로그램 코드만 교체합니다. 설치 뒤 대시보드에 로그인한 후 관리 페이지에서 Slack 알림 채널, 채용별 면접관 일정 요청 채널, Codex MCP 연결을 확인합니다.

업데이트 설치 전에는 기존 SQLite DB의 안전한 스냅샷을 `data/backups`에 먼저 생성합니다. 워커와 대시보드 실행 로그는 설치 폴더의 `logs/worker.log`, `logs/dashboard.log`에서 확인할 수 있습니다. 설치 파일은 개인용 초기 설정값과 DB를 포함할 수 있으므로 GitHub·메신저·공용 저장소에 올리지 않고, 지정된 담당자 PC에만 전달합니다. 설치 뒤에는 현재 Windows 사용자와 시스템 계정만 설치 폴더를 읽을 수 있도록 권한을 제한합니다.

## 대시보드 보안

- 대시보드 화면과 API는 단일 관리자 로그인 세션으로 보호합니다.
- 비밀번호는 평문이 아닌 `scrypt` 해시로 로컬 DB에 저장합니다.
- 로그인 세션은 12시간 후 만료되며, 로그아웃 또는 비밀번호 변경 시 폐기됩니다.
- 로그인 실패는 15분 동안 최대 5회만 허용합니다.
- 변경 API는 동일 출처 요청만 허용하고, 대시보드 응답은 캐시·iframe 삽입을 막는 보안 헤더를 적용합니다.
- API 키, Slack 토큰, DB, 브라우저 로그인 프로필은 Git에 올리지 않습니다.

초기 로그인 후에는 관리 페이지에서 비밀번호를 즉시 변경하세요. 이 도구는 로컬 PC용이므로 Windows 계정 잠금과 디스크 암호화 같은 PC 자체 보안도 함께 적용해야 합니다.

## 채용을 새로 추가할 때

1. 관리 페이지에서 진행 중인 나인하이어 채용을 불러옵니다.
2. 해당 채용의 면접관 일정 요청용 Slack 채널 ID를 연결합니다.
3. 채용의 칸반 단계와 인터뷰 유형을 확인하고, 일반·통합·연속·30분 인터뷰 중 맞는 규칙을 승인합니다.
4. 이후 후보자가 조율 대상이 되면 대시보드 운영 큐에서 승인부터 진행합니다.

채용별 Slack 채널을 지정하지 않은 채용에는 테스트용 채널로 메시지를 보내지 않습니다. 반드시 해당 채용과 실제 운영 채널을 연결해야 합니다.
...
> 현재 단계는 **MCP 서버 + Slack 테스트 앱 + 로컬 워커 + 다우오피스 회의실 조회 + 로컬 운영 대시보드 + 나인하이어 일정 제안 발송 승인**입니다. 후보자 메일은 대시보드에서 제목·후보일·장소·내부 참석자·회신 기한·이메일 템플릿을 확인하고 `나인하이어 메일 발송`을 명시적으로 승인할 때만 전용 Chrome으로 발송합니다. 비밀번호는 저장하지 않습니다.
...
- 후보자 일정 제안은 채용명에서 고용형태 표기만 뺀 제목, 고정 안내 문구·장소·회신 기한·이메일 템플릿 규칙을 적용합니다. 기본은 60분이며, 대기업·공공기업 영업의 1day 인터뷰는 120분, AI 강사 인터뷰는 30분입니다. 의문당과 8층 회의실처럼 안내 장소가 다른 후보일을 한 번에 선택하면 잘못된 장소 안내를 막기 위해 발송 전 중단합니다.
- 발송 성공이 확인된 뒤에만 로컬 상태를 `후보자 응답 대기`로 바꿉니다. 후보자 회신이 불명확하면 재조율·취소·보류·직접 확인 중 하나를 선택하도록 대시보드에 남기며 자동 취소하지 않습니다.
...
- 대시보드 API에는 단일 관리자 로그인과 CSRF 방어를 적용했습니다. 여러 담당자가 동시에 쓰는 중앙 운영으로 전환할 때는 역할 분리와 승인자 감사 기록을 추가합니다.
...
- 다우오피스 예약 생성·수정·취소는 지원하지 않습니다. 나인하이어 후보자 일정 제안 메일은 대시보드에서 내용을 확인하고 명시적으로 승인한 경우에만 로그인된 전용 Chrome으로 처리합니다.
...
브라우저에서 `http://127.0.0.1:3100`을 엽니다. 이 버전은 한 대의 PC에서 담당자 한 명이 사용하는 파일럿 운영용이며, 단일 관리자 로그인으로 보호합니다.
...
- Codex 개인 스킬 `$ninehire-schedule-proposal`은 후보일 추천, 나인하이어 일정 조율 메일의 사전 검토·발송 승인, 후보자 회신 후 재조율까지를 같은 규칙으로 수행합니다. 세부 MCP 도구를 대체하지 않고 올바른 순서로 조합합니다.
...
- 후보자에게 인터뷰 일정을 생성하거나 메시지를 발송하는 나인하이어 MCP 도구는 현재 확인되지 않았습니다. 따라서 이 프로젝트는 읽기에는 나인하이어 MCP를 사용하고, 일정 제안 메일은 사용자가 대시보드에서 명시적으로 승인한 경우에만 로그인된 전용 Chrome으로 처리합니다.
