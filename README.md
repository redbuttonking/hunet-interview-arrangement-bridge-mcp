...
3. 로그인한 Edge 창은 열어 둔다. 이후 브리지 서버가 `127.0.0.1`에만 열린 로컬 연결을 통해 예약 화면을 읽는다.

브라우저 프로필은 기본적으로 `data/daou-office-profile`에 저장되며 Git에서 제외된다. 로그인 세션이 만료되면 같은 전용 Edge 창에서 다시 로그인하면 된다.
...
- 후보자 카드를 열면 인터뷰 유형, 일정·회의실, 면접관 제출 상태, Slack 초안 상태, 업무 이력을 볼 수 있습니다.
...
- 외부 데이터 신선도에는 화면을 연 시각이 아니라 Slack 알림, 나인하이어 일정, 다우오피스 회의실, 워커의 마지막 성공 시각을 따로 표시합니다. 10분 이상 성공 기록이 없으면 최신 확인이 필요한 상태로 표시합니다.
...
- 워커 단일 실행 lease, 리마인더 발송 claim, Slack·나인하이어·다우오피스 장애 통합 테스트를 추가합니다. 작업 스케줄러를 두 번 실행해도 중복 발송되지 않아야 합니다.
...
- `READY_TO_SCHEDULE` 상태에서 사용자가 나인하이어로 직접 확정한 일정은 Slack 확정 알림 또는 5분 주기의 나인하이어 일정 조회로 기록한다. 조회 대상은 이미 로컬에서 추적 중인 조율 건과 조율 시작 승인 대기 후보자로 제한한다.
